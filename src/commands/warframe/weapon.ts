...
import { clean, getNumberReactions, levenshtein, round, search } from "../../utils";
...
function displayCritical({ criticalChance, criticalMultiplier }: Weapon) {
    ...
    const critChance = round(Number(criticalChance) * 100);
...
function displayStatus({ procChance, damageTypes }: Weapon) {
    const statusChance = round(Number(procChance) * 100);
