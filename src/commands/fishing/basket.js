...
module.exports = new Command({
    ...
    async action(msg) {
        ...
        if (mention) {
            if (mention.id === Salty.bot.user.id) {
                throw new error.SaltyException(
                    "I don't have a basket. Fishes are friends, not food !"
                );
            }
        ...
        let options = {
            title: title,
            color:
                Salty.config.quality[Salty.config.rank[userStats.rank].quality]
                    .color,
            fields: [
                {
                    title: `Rank ${userStats.rank}: ${
                        Salty.config.rank[userStats.rank].name
                    }`,
                    description: `current XP: ${Math.floor(
                        Salty.getXpInfos(reqUserId).xp
                    )}/${Salty.config.rank[userStats.rank].xp}`,
                },
                {
                    title: "fish count: " + userStats.fishCount,
                    description:
                        userStats.fishCount < 20
                            ? "that's not much"
                            : "that's a lot of fishes !",
                },
                {
                    title: "time spent fishing",
                    description: UTIL.formatDuration(userStats.fishingTime),
                },
            ],
            inline: true,
        };
