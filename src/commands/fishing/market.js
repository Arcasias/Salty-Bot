...
module.exports = new Command({
    ...
    async action(msg, args) {
        ...
        if (args[0] && Salty.getList("buy").includes(args[0])) {
            let itemIds = Object.keys(items).filter(
                (itemId) => items[itemId].price
            );
            let marketItems = itemIds.map((id) =>
                Object.assign(items[id], { id })
            );
            let options = {
                title: "sea market",
                color: 0xffffff,
            ...
            if (inventory.includes(marketItems[args[1]].id)) {
                throw new error.SaltyException(
                    "you already have one of these. Why buy another ?"
                );
            }
            ...
            Salty.embed(msg, {
                ...
                }** successfully sold **${item.name}** for ${
                    item.price * repurchase
                } gold`,
                color: 0xffffff,
                inline: true,
                fields: [],
            ...
            if (args[0]) {
                ...
                Salty.embed(msg, {
                    title: "sea market",
                    description:
                        "welcome, customer ! Have a look on my beautiful wares !",
                    color: 0xffffff,
                    inline: true,
                    fields,
