...
class ObjectCommand extends Command {
    ...
    constructor(data) {
        ...
        this.list = data.list;
        this.constraint =
            data.constraint || ((msg, args) => args.slice(1).join(" "));
    }
    ...
    _run(msg, args) {
        ...
        if (args[0]) {
            ...
            if (Salty.getList("add").includes(args[0])) {
                if (!this.perm[PERM_SET])
                    throw new error.PermissionDenied("create");
                ...
                Salty.embed(msg, {
                    title: `${args.join(" ")} added to ${this.name}`,
                    type: "success",
                });
                ...
                if (!this.perm[PERM_SET])
                    throw new error.PermissionDenied("delete");
                if (!args[1]) throw new error.MissingArg("element");
                ...
                if (argIsIndex) {
                    ...
                    elementIndex = this.list.indexOf(
                        this.list.find((el) =>
                            Object.values(el).includes(joined)
                        )
                    );
                }

                if (!this.list[elementIndex])
                    throw new error.OutOfRange(elementIndex);

                this.list.splice(elementIndex, 1);

                Salty.embed(msg, {
                    title: `${
                        argIsIndex ? "item number " + args[1] : joined
                    } removed from ${this.name}`,
                    type: "success",
                });
                ...
                if (!this.perm[PERM_SET])
                    throw new error.PermissionDenied("clear");

                this.list = [];

                Salty.embed(msg, {
                    title: `${this.name} cleared`,
                    type: "success",
                });
            }
        } else {
