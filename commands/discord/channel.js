    ...
    async action(msg, args) {
        ...
        if (args[0] && Salty.getList('add').includes(args[0])) {
            ...
            if (parseInt(msg.channel.id) === parseInt(guild.default_channel)) {
                await Salty.embed(msg, { title: "this is the current default channel", description: "I'll speak right here when I need to" });
            } else {
                await Salty.embed(msg, { title: `default bot channel is **${chanName}**`, description: "this is where I'll speak when I need to" });
            }
        }
    },
});
